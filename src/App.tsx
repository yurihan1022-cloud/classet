...
import { Crown, Lock, User, Check, Trash2, Award, UserPlus, Info, AlertCircle, X, Unlock, Clock, Power } from 'lucide-react';

// ==========================================
// [1] 파이어베이스 시스템 키값 (절대 수정 금지)
// 선생님의 스크린샷에 있던 진짜 키값들을 적용해 두었습니다.
// ==========================================
...
export default function App() {
  ...
  const [votingStatus, setVotingStatus] = useState({ president: false, vice: false }); // false: 진행중, true: 마감됨
  ...
  const currentCandidates = useMemo(() => {
    return candidates.filter(c => c.type === electionType).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
    ...
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-20">
      ...
      <header className="bg-white shadow-sm sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-4 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-lg w-full sm:w-auto">
            <button
              onClick={() => { setElectionType('president'); setActiveTab(isAdmin && activeTab === 'admin' ? 'admin' : 'vote'); }}
              className={`flex-1 sm:flex-none px-4 py-2 rounded-md text-sm font-medium transition-colors ${electionType === 'president' ? 'bg-white shadow text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
            >
              반장 선거
            </button>
            <button
              onClick={() => { setElectionType('vice'); setActiveTab(isAdmin && activeTab === 'admin' ? 'admin' : 'vote'); }}
              className={`flex-1 sm:flex-none px-4 py-2 rounded-md text-sm font-medium transition-colors ${electionType === 'vice' ? 'bg-white shadow text-blue-600' : 'text-slate-500 hover:text-slate-700'}`}
            >
              부반장 선거
            </button>
          </div>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex justify-center mb-8 border-b border-slate-200">
          <button
            ...
            className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors ${activeTab === 'vote' ? 'border-blue-500 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            투표하기
          </button>
          <button
            onClick={() => setActiveTab('results')}
            className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors ${activeTab === 'results' ? 'border-blue-500 text-blue-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
          >
            결과보기
          </button>
          {isAdmin && (
            <button
              ...
              className={`px-4 sm:px-6 py-3 font-medium border-b-2 transition-colors flex items-center gap-1 ${activeTab === 'admin' ? 'border-red-500 text-red-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
            >
        ...
        <div className="mb-6 flex justify-between items-end">
          <div>
            <h2 className="text-xl font-bold text-slate-700 flex items-center gap-2 mb-2">
              ...
              {isAdmin && <span className="text-sm bg-red-100 text-red-600 px-2 py-1 rounded-full ml-2">관리자 모드</span>}
            </h2>
            <p className="text-slate-500 text-sm">
              {activeTab === 'vote' && "소중한 한 표를 행사해주세요. 누구에게 투표했는지는 선생님만 알 수 있습니다!"}
              {activeTab === 'results' && "현재까지의 투표 결과입니다."}
              {activeTab === 'admin' && "투표를 마감하거나 후보를 관리할 수 있습니다."}
            </p>
          </div>
          
          <div className="hidden sm:flex items-center">
             <span className={`px-3 py-1.5 rounded-full text-xs font-bold flex items-center gap-1.5 shadow-sm ${isCurrentElectionClosed ? 'bg-slate-700 text-white' : 'bg-green-100 text-green-700'}`}>
                {isCurrentElectionClosed ? <Lock className="w-3.5 h-3.5" /> : <Unlock className="w-3.5 h-3.5" />}
        ...
        <div className="sm:hidden mb-6">
           <span className={`inline-flex px-3 py-1.5 rounded-full text-xs font-bold items-center gap-1.5 shadow-sm ${isCurrentElectionClosed ? 'bg-slate-700 text-white' : 'bg-green-100 text-green-700'}`}>
              {isCurrentElectionClosed ? <Lock className="w-3.5 h-3.5" /> : <Unlock className="w-3.5 h-3.5" />}
              {isCurrentElectionClosed ? '투표 마감됨' : '투표 진행중'}
           </span>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-8">
            {isCurrentElectionClosed ? (
              <div className="text-center py-12 flex flex-col items-center">
                ...
                <p className="text-slate-500 mb-6">참여해주셔서 감사합니다. 상단의 '결과보기' 탭에서 결과를 확인하세요!</p>
                <button 
                  onClick={() => setActiveTab('results')}
              ...
              <form onSubmit={handleVote} className="space-y-6 animate-in fade-in duration-300">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">1. 본인의 학번을 적어주세요</label>
                  <input
                    type="text"
                    required
                    placeholder="예: 10101"
                  ...
                  <p className="text-xs text-red-500 mt-2 flex items-center gap-1">
                    <Lock className="w-3 h-3" /> 학번은 선생님만 볼 수 있어요! 안심하세요.
                  </p>
                </div>

                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-3">2. 투표할 후보를 선택하세요</label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {currentCandidates.map(c => (
                      <label 
                        key={c.id} 
                        className={`flex items-center p-4 rounded-xl border-2 cursor-pointer transition-all ${selectedCandidate === c.id ? 'border-blue-500 bg-blue-50 text-blue-700 shadow-sm' : 'border-slate-100 hover:border-blue-200 hover:bg-slate-50'}`}
                      >
                        <input
                          type="radio"
                          name="candidate"
                        ...
                        />
                        <span className="ml-3 font-medium text-lg">{c.name}</span>
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">3. 이 후보를 뽑은 이유는 무엇인가요?</label>
                  <textarea
                    required
                  ...
                  <div key={result.id} className={`bg-white rounded-2xl shadow-sm border p-6 relative overflow-hidden ${result.isWinner && isCurrentElectionClosed ? 'border-yellow-400 ring-4 ring-yellow-400 ring-opacity-30' : 'border-slate-100'}`}>
                    ...
                    <div className="flex items-center justify-between mb-4 mt-2">
                      ...
                      <div className="text-center bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
                        <span className={`text-3xl font-black ${result.isWinner && isCurrentElectionClosed ? 'text-yellow-500' : 'text-blue-600'}`}>
                          {result.voteCount}
                        </span>
                        <span className="text-slate-500 ml-1 font-medium">표</span>
                      </div>
                    </div>
                      ...
                      {result.voterReasons.length > 0 ? (
                        <ul className="space-y-2">
                          {result.voterReasons.map((v, idx) => (
                            <li key={idx} className="text-slate-700 bg-white p-3 rounded-lg border border-slate-100 text-sm shadow-sm flex flex-col sm:flex-row gap-2">
                              ...
                              <span className="flex-1 leading-relaxed">"{v.reason}"</span>
                            </li>
                          ))}
                        </ul>
                      ) : (
                        <p className="text-sm text-slate-400 italic">아직 투표가 없습니다.</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        {activeTab === 'admin' && isAdmin && (
          <div className="space-y-6">
            ...
            <div className={`rounded-2xl shadow-sm border p-6 flex flex-col sm:flex-row items-center justify-between gap-4 ${isCurrentElectionClosed ? 'bg-slate-50 border-slate-200' : 'bg-blue-50 border-blue-200'}`}>
               <div>
                 <h3 className="text-lg font-bold flex items-center gap-2 text-slate-800 mb-1">
                   {isCurrentElectionClosed ? <Lock className="w-5 h-5 text-slate-500" /> : <Unlock className="w-5 h-5 text-blue-500" />}
                   투표 진행 상태 관리
                 </h3>
                 <p className="text-sm text-slate-500">
                   {isCurrentElectionClosed ? '학생들의 투표가 제한되며, 아이들도 결과를 볼 수 있습니다.' : '학생들이 투표를 할 수 있으며, 결과는 선생님만 볼 수 있습니다.'}
                 </p>
               </div>
               
               <button 
                 onClick={toggleVotingStatus}
                 className={`flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-bold text-white transition-all shadow-sm w-full sm:w-auto ${isCurrentElectionClosed ? 'bg-slate-700 hover:bg-slate-800' : 'bg-red-500 hover:bg-red-600'}`}
               >
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2 text-slate-800">
                <UserPlus className="w-5 h-5 text-blue-500" /> 후보 관리 (최대 5명)
              </h3>
              ...
              <form onSubmit={handleAddCandidate} className="flex gap-2 mb-6">
                ...
                <button
                  type="submit"
                  disabled={currentCandidates.length >= 5 || isCurrentElectionClosed}
              ...
              {isCurrentElectionClosed && <p className="text-xs text-red-500 mb-4">* 투표가 마감된 상태에서는 후보를 변경할 수 없습니다.</p>}

              <ul className="space-y-2">
                {currentCandidates.map(c => (
                  <li key={c.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg border border-slate-100">
                ...
                {currentCandidates.length === 0 && (
                  <p className="text-slate-400 text-sm text-center py-4">등록된 후보가 없습니다.</p>
                )}
              </ul>
            </div>
            ...
            <div className="bg-yellow-50 rounded-2xl shadow-sm border border-yellow-100 p-6">
              ...
              <ul className="text-sm text-yellow-700 leading-relaxed list-disc list-inside space-y-1">
                <li>상단의 <strong>'결과보기'</strong> 탭으로 이동하시면 실시간 결과와 <b>학생들의 학번</b>을 확인할 수 있습니다.</li>
                <li>학생들에게 결과를 공개하려면 반드시 <b>[투표 마감하기]</b> 버튼을 눌러주세요.</li>
              </ul>
              <button 
      ...
      {showAdminLogin && !isAdmin && (
        <div className="fixed inset-0 bg-slate-900 bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl relative animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-sm text-slate-500 mb-5">선생님 전용 메뉴입니다. 비밀번호를 입력해주세요.</p>
      ...
      {toast && (
        <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 z-50 animate-in slide-in-from-bottom-5">
          <div className={`${toast.type === 'error' ? 'bg-red-600' : 'bg-slate-800'} text-white px-6 py-3 rounded-full shadow-lg font-medium text-sm flex items-center gap-2 whitespace-nowrap`}>
